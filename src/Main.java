...
public class Main {
    ...
    private static void check() throws InterruptedException {
        ...
        List<CorrectnesCheckExecutor> executors = range(0, THREAD_COUNT)
                .mapToObj(i -> new CorrectnesCheckExecutor(tree, inserts, THREAD_COUNT, i))
                .collect(toList());
        executors.forEach(CorrectnesCheckExecutor::start);
        for (CorrectnesCheckExecutor e : executors) e.join();

        executors = range(0, THREAD_COUNT)
                .mapToObj(i -> new CorrectnesCheckExecutor(tree, removes, THREAD_COUNT, i))
                .collect(toList());
        executors.forEach(CorrectnesCheckExecutor::start);
        for (CorrectnesCheckExecutor e : executors) e.join();
